...
import de.bno.jellysplush.gui.start.ColorPicker;
...
public class EndScene implements Scene, KeyListener {
	...
	public EndScene(JellyFish fish1, JellyFish fish2, boolean rightIsWinner) {
		...
		if (rightIsWinner) {
			...
			winText = String.format(
					"The winner is, with %d points and %d deaths ...",
					fish2.getPoints(), -fish2.getLifes());
			...
			winText = String.format(
					"The winner is, with %d points and %d deaths ...",
					fish1.getPoints(), -fish1.getLifes());
	...
	private void drawStatus(Graphics2D g, int width, int height) {
		...
		cp.setTopLeftPosition(new Point(
				(int) (width * 0.5 - cp.getWidth() * 0.5),
				(int) (height * 0.5 - cp.getHeight() * 0.5)));
		cp.paintOnScene(g, 0);
		...
		while (mHeight > _height || mWidth > _width) {
			...
			oldFont = g.getFont();
			g.setFont(new Font(oldFont.getName(), oldFont.getStyle(), oldFont
					.getSize() - 1));
		...
		g.drawString(winText, (float) (_width * 0.5 - mWidth * 0.5),
				(float) (_height * 0.5 - mHeight * 0.5));
	}
