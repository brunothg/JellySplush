...
public class StartScene implements Scene, KeyListener
{
	...
	private static final Color BACKGROUND_COLOR_1 = Constants.START_COLOR_1;
	private static final Color BACKGROUND_COLOR_2 = Constants.START_COLOR_2;

	private static final int BACKGROUND_HEIGHT = 20;

	private static final Color[] colors = min(
		Settings.getColorArray(Settings.KEY_USER_COLORS, Constants.USER_COLORS_DEFAULT), 2,
	...
	private static Color[] min(Color[] colorArray, int min, Color[] userColorsDefault)
	{
	...
	public void paintScene(Graphics2D g2d, int width, int height, long elapsed)
	{
		...
		if (countdown.isRunning())
		{
			...
			countdown.setTopLeftPosition(new Point(0, 0));
			countdown.paintOnScene(g2d, elapsed);

			if (countdown.getNumber() < 0)
			{
